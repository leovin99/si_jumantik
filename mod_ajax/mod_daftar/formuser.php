...
$(function(){
	...
	function loadData(){
		page="mod_ajax/mod_daftar/page_user.php";
		...
		if (((vUsername.replace(/\s/g,"") == "") || (vNama.replace(/\s/g,"") == "") || (vPassword.replace(/\s/g,"") == "") || (vPassConfirm.replace(/\s/g,"") == "")) && vHidden == "add") {
		  ...
		  $("input#pass").focus();
			...
			if (confirm("Apakah benar akan menyimpan data user ini?")){
				$.ajax({
					url: "mod_ajax/mod_daftar/proses_user.php",
					...
					success:function(response){
						if(response.status == 1){
							...
							page="mod_ajax/mod_daftar/formuser.php";
	...
	<div class="col-lg-12">
		<div class="panel panel-default">
			...
			<div class="panel-body">
				<div class="row">
					<div class="col-lg-6">
						<form method="post" name="formUser" action="http://192.168.253.96/uas%20-%20copy/admin/index.php" id="formUser" role="form">
							...
							<div class="form-group">
								...
								<select id="ktrole" name="ktrole" readonly class="form-control">
									<option value='operator'<?php if ($tipeuser == 'operator') echo " selected";?>>Operator</option>
								</select>
							</div>
							<div class="form-group">
								<label>Status</label>
								<select id="ktstatus" name="ktstatus" readonly class="form-control">
