<?php 
	...
	$desa = cleanall($_POST['desa']);
	$banjar = cleanall($_POST['banjar']);
	...
	if($action=="add") 
	{
		...
		if($cekkode && $cekkode->num_rows == 1){
			...
			$cekq = $mysqli->query("INSERT INTO tb_users(username,password,name,type,desa,banjar,status_blok) VALUES('$uname','$password','$nama','$ktrole','$desa','$banjar','$ktstatus')") or die ("query gagal dijalankan: ".$mysqli->error);
	...
	else if($action=="update") 
	{
		if (isset($pass) && !empty($pass)) {
			...
			$cekq = $mysqli->query("UPDATE tb_users SET password='$password',name='$nama',type='$ktrole',desa='$desa',banjar='$banjar',status_blok='$ktstatus' where username='$uname'") or die ("query gagal dijalankan: ".$mysqli->error);
		}else{
			$cekq = $mysqli->query("UPDATE tb_users SET name='$nama',type='$ktrole',desa='$desa',banjar='$banjar',status_blok='$ktstatus' where username='$uname'") or die ("query gagal dijalankan: ".$mysqli->error);
		}
